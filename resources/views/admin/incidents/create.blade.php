...
@section('page-description', 'Record a new incident report')

@section('content')
<div class="container-fluid">
    <div class="form-container">
        <div class="form-header">
            <h2><i class="fas fa-exclamation-triangle"></i> New Incident</h2>
            <a href="{{ route('admin.incidents.index') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left"></i> Back
            </a>
        </div>

        <form action="{{ route('admin.incidents.store') }}" method="POST" class="form-main">
            @csrf
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="incident_number" class="form-control" value="{{ old('incident_number') }}" placeholder="Leave blank to auto-generate">
                        @error('incident_number')<span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group">
                        <label>Incident Date <span class="text-danger">*</span></label>
                        <input type="datetime-local" name="incident_date" class="form-control" value="{{ old('incident_date', now()->format('Y-m-d\\TH:i')) }}" required>
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="affected_user_id" class="form-control">
                            <option value="">None</option>
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('affected_user_id') == $user->id ? 'selected' : '' }}>
                                    {{ trim(($user->first_name ?? '') . ' ' . ($user->last_name ?? '')) ?: $user->username }} ({{ $user->role }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="location" class="form-control" value="{{ old('location') }}" maxlength="150" required>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea name="root_cause" class="form-control" rows="3">{{ old('root_cause') }}</textarea>
                    @error('root_cause')<span class="text-danger">{{ $message }}</span>@enderror
                </div>

                <div class="form-group">
                    <label>Corrective Action</label>
                    <textarea name="corrective_action" class="form-control" rows="3">{{ old('corrective_action') }}</textarea>
