...
@section('page-description', 'Register a new pet in the system')
...
<div class="form-container">
    <h2 style="margin-top: 0; color: var(--dark-text); font-family: 'Fredoka', sans-serif;">Register New Pet</h2>
    
    <form action="{{ route('admin.pets.store') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <!-- Owner Selection -->
        <div class="form-group {{ $errors->has('owner_id') ? 'error' : '' }}">
            <label for="owner_id">Pet Owner <span style="color: var(--accent-pink);">*</span></label>
            ...
            @if($errors->has('owner_id'))
                <div class="error-message">{{ $errors->first('owner_id') }}</div>
            @endif
        </div>
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn btn-primary">
                <i class="fas fa-save"></i> Register Pet
            </button>
        </div>
    </form>
</div>
